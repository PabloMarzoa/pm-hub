...
export class PokemonApiService extends PmdsCdkTableCompleteDataService {
            ...
            .subscribe((res: PokemonResponse | any) => {
                const result: IPmdsCdkTableCompleteResponseModel<{name: string, url: string}> = {
                    ...
                    registers: res.results.map((el: PokemonItem) => {
                        return {name: {label: el.name, sublabel: el.name === 'bulbasaur'}, url: el.url, disabledAction: el.name === 'bulbasaur'}
                    })
                };
